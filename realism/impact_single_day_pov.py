...
def make_plots(plot_data, plot_params_dict):
    ...
    orderbook_df["MID_PRICE"].plot(
        ax=axes[0],
        label=plot_params_dict["baseline_label"],
        color=color,
        linestyle=linestyle,
    )
    ...
    for plot_data_dict in plot_data:
        ...
        orderbook_with_execution_df["MID_PRICE"].plot(
            ax=axes[0],
            label=f'{plot_params_dict["execution_label"]}{pov}',
            color=color,
            linestyle=linestyle,
        )
...
def prep_data(
    ...
    for data_dict in plot_data:
        ...
        abides_orderbook_df = make_orderbook_for_analysis(
            data_dict["no_execution_exchange_path"],
            data_dict["no_execution_orderbook_path"],
            num_levels=1,
        )
        abides_execution_orderbook_df = make_orderbook_for_analysis(
            data_dict["yes_execution_exchange_path"],
            data_dict["yes_execution_orderbook_path"],
            num_levels=1,
        )
        ...
        shade_start_time, shade_end_time = prepare_shade_dates(
            plot_params_dict["shade_start_datetime"],
