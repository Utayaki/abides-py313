...
parser.add_argument("-e", "--epsilon", type=float, default=1.0, help="Privacy loss epsilon")
parser.add_argument(
    "-g",
    "--num_subgraphs",
    type=int,
...
parser.add_argument("-k", "--skip_log", action="store_true", help="Skip writing agent logs to disk")
parser.add_argument(
    "-l",
    "--log_dir",
    default=None,
...
for i in range(a, b):
    ...
    agents.append(
        PPFL_ClientAgent(
            i,
            "PPFL Client Agent {}".format(i),
            "PPFL_ClientAgent",
            peer_list=[x for x in neighbors if x != i],
            iterations=num_iterations,
            max_logreg_iterations=max_logreg_iterations,
            epsilon=epsilon,
...
print()
print(
    f"Protocol Iterations: {num_iterations}, Clients: {num_clients}, Split Size: {split_size}, "
