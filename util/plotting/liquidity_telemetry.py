...
def bin_and_sum(s, binwidth):
    ...
    bins = pd.interval_range(
        start=s.index[0].floor("min"),
        end=s.index[-1].ceil("min"),
...
def print_liquidity_stats(
    transacted_orders,
    no_bid_idx,
    no_ask_idx,
    liquidity_dropout_buffer=LIQUIDITY_DROPOUT_BUFFER,
):
...
def make_plots(plot_inputs, plot_params_dict, title=None, out_file="liquidity_telemetry.png"):
    ...
    # axes[0].legend(fontsize='large')
    axes[2].set_ylabel(
        r"$\frac{\mathrm{best\ ask\ size}}{\mathrm{best\ ask\ size} + \mathrm{best\ bid\ size}}$",
        fontsize="large",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "stream",
        type=str,
        help="ABIDES order stream in bz2 format. " "Typical example is `ExchangeAgent.bz2`",
    )
    parser.add_argument(
        "book",
        type=str,
        help="ABIDES order book output in bz2 format. Typical example is " "ORDERBOOK_TICKER_FULL.bz2",
    )
    parser.add_argument(
        "-o",
        "--out_file",
