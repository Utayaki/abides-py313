...
class SumClientAgent(Agent):
    ...
    def kernelStarting(self, startTime):
        ...
        log_print(
            "Agent {} requested agent of type Agent.SumServiceAgent.  Given Agent ID: {}",
            self.id,
            self.serviceAgentID,
        )
    ...
    def wakeup(self, currentTime):
        ...
        if not self.peer_exchange_complete:
            ...
            for idx, peer in enumerate(self.peer_list):
                self.sendMessage(
                    peer,
                    Message({"msg": "PEER_EXCHANGE", "sender": self.id, "n": n[idx]}),
                )
            ...
            log_print(
                "agent {} transmitting numbers {} and {} with peer sum {}",
                self.id,
                n1,
                n2,
                self.peer_sum,
            ...
            self.sendMessage(
                self.serviceAgentID,
                Message({"msg": "SUM_QUERY", "sender": self.id, "n1": n1, "n2": n2}),
            )
