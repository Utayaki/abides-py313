...
class PPFL_ClientAgent(Agent):
    ...
    def __init__(
        ...
        self.elapsed_time = {
            "DH_OFFLINE": pd.Timedelta(0),
            "DH_ONLINE": pd.Timedelta(0),
            "TRAINING": pd.Timedelta(0),
            "ENCRYPTION": pd.Timedelta(0),
        }
        ...
        if self.id == 1:
            print(f"px_shape is {self.px_shape}")
        if self.id == 1:
            print(f"px_scale is {self.px_scale}")
    ...
    def wakeup(self, currentTime):
        ...
        if not self.peer_exchange_complete:
                ...
                for idx, peer in enumerate(self.peer_list):
                    ...
                    self.sendMessage(
                        ...
                        Message(
                            {
                                "msg": "FWD_MSG",
                                "msgToForward": "PEER_EXCHANGE",
                                "sender": self.id,
                                "recipient": peer,
                                "pubkey": self.pubkeys[peer],
                            }
                        ),
                    )
            ...
            if self.current_iteration == 0:
                ...
                self.my_noise = np.random.laplace(
                    scale=self.px_scale,
                    size=(self.no_of_iterations, self.no_of_weights),
                )
            ...
            weight = getWeights(
                ...
                trainX=self.trainX[i],
                trainY=self.trainY[i],
                self_id=self.id,
            )
            ...
            if self.clear_learning:
                ...
                n = (np.array(weight) + noise) * self.multiplier
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if msg.body["msg"] == "PEER_EXCHANGE":
            ...
            log_print(
                "Client weights received for iteration {} by {}: {}",
            ...
            if self.id == 1:
                ...
                if not exists("results/all_results.csv"):
                    with open("results/all_results.csv", "a") as results_file:
                        ...
                        results_file.write(
                            f"Clients,Peers,Subgraphs,Iterations,Train Rows,Learning Rate,In The Clear?,Local Iterations,Epsilon,Iteration,IS ACC,OOS ACC,IS MCC,OOS MCC,IS MSE,OOS MSE,IS F1,OOS F1,IS AUPRC,OOS AUPRC\n"
                        )

                with open("results/all_results.csv", "a") as results_file:
                    # Write out the current protocol iteration weights and metadata.
                    results_file.write(
                        f"{self.num_clients},{self.num_peers},{self.num_subgraphs},{self.no_of_iterations},{self.split_size},{self.learning_rate},{self.clear_learning},{self.max_logreg_iterations},{self.epsilon},{self.current_iteration},{is_acc},{oos_acc},{is_mcc},{oos_mcc},{is_mse},{oos_mse},{is_f1},{oos_f1},{is_auprc},{oos_auprc}\n"
                    )
