...
class EtfPrimaryAgent(FinancialAgent):
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if currentTime > self.prime_close:
            ...
            log_print(
                "{} received {}, discarded: prime is closed.",
                self.name,
                msg.body["msg"],
            )
        ...
        if msg.body["msg"] == "WHEN_PRIME_OPEN":
            log_print(
                "{} received WHEN_PRIME_OPEN request from agent {}",
                self.name,
                msg.body["sender"],
            )
            ...
            self.sendMessage(
                msg.body["sender"],
                Message({"msg": "WHEN_PRIME_OPEN", "data": self.prime_open}),
            )

        elif msg.body["msg"] == "WHEN_PRIME_CLOSE":
            log_print(
                "{} received WHEN_PRIME_CLOSE request from agent {}",
                self.name,
                msg.body["sender"],
            )
            ...
            self.sendMessage(
                msg.body["sender"],
                Message({"msg": "WHEN_PRIME_CLOSE", "data": self.prime_close}),
            )

        elif msg.body["msg"] == "QUERY_NAV":
            log_print(
                "{} received QUERY_NAV ({}) request from agent {}",
                self.name,
                msg.body["sender"],
            )
